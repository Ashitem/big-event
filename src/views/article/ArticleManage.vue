...
const articleList = async () => {
  ...
  //处理数据，给数据模型拓展一个属性categoryName（分类名称）
...
const uploadSuccess = (result) => {
  if (result.code == 0) {
    ...
    ElMessage.warning(result.message ? result.message : '服务异常');
  } else {
...
const clearData = () => {
  ...
  articleModel.value.content = ' ';//添加空格占位，达到清空的效果
  ...
  <el-card class="page-container">
    ...
    <el-drawer v-model="visibleDrawer" :title="title" direction="rtl" size="50%">
      ...
      <el-form :model="articleModel" label-width="100px">
        ...
        <el-form-item>
          <el-button type="primary" @click="title=='添加文章'?addArticle('已发布'):updateArticle('已发布')">发布
          </el-button>
